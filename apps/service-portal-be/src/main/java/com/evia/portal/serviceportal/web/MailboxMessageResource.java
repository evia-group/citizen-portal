...
public class MailboxMessageResource {
  ...
  public ResponseEntity<List<MailboxMessageDTO>> getMailboxMessages(
    @Parameter(description = "Get mailbox log according to a profile by passing the profile id") @RequestParam(name = "profileId", required = false) Long profileId,
    @Parameter(description = "Get mailbox log according to a application by passing the application id") @RequestParam(name = "applicationId", required = false) Long applicationId
  ) {
