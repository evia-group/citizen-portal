...
import com.evia.portal.adminportal.core.repository.*;
...
public class ConsentResourceIntegrationTest {
  ...
  void setUp() throws Exception {
    ...
    createdConsents = persistTestProfile(testConsents);
  }
  ...
  private List<ConsentDTO> persistTestProfile(List<ConsentDTO> consentDTOs) throws Exception {

    List<ConsentDTO> savedConsents = new ArrayList<>();

    for (ConsentDTO consentDTO : consentDTOs) {

      MvcResult result = mockMvc.perform(post("/api/v1/consents")
          .content(objectMapper.writeValueAsString(consentDTO))
          .contentType(MediaType.APPLICATION_JSON))
        .andReturn();

      String json = result.getResponse().getContentAsString();
      savedConsents.add(objectMapper.readValue(json, ConsentDTO.class));
    }
