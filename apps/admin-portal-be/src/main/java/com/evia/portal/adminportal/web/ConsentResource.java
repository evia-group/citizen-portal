...
import com.evia.portal.adminportal.core.service.ConsentService;
import com.evia.portal.adminportal.web.mapper.ConsentMapper;
...
public class ConsentResource {
  ...
  public ResponseEntity<ConsentDTO> createConsent(@RequestBody ConsentDTO consentDTO) {
    ...
    final Consent consent = consentMapper.toConsent(consentDTO);
    final Consent createdConsent = consentService.createConsent(consent);

    final ConsentDTO createdConsentDTO = consentMapper.toConsentDTO(createdConsent);

    return ResponseEntity.ok(createdConsentDTO);
  }

  @PutMapping(path = "{id}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<ConsentDTO> updateService(@RequestBody ConsentDTO consentDTO, @PathVariable("id") Long id) {

    final Consent consent = consentMapper.toConsent(consentDTO);
    final Consent updatedConsent = consentService.updateConsent(consent, id);

    final ConsentDTO updatedConsentDTO = consentMapper.toConsentDTO(updatedConsent);

    return ResponseEntity.ok(updatedConsentDTO);
  }
}
