...
#
# Notes:
#   - Port 80 is privileged, so socat runs under sudo (the devcontainer grants vscode
#     passwordless sudo). Forwards are started detached (setsid) and left running for the
#     container lifetime — there is no teardown.
